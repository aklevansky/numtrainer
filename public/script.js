 ...
 */
var reInterpolate = __webpack_require__(64),
...
var _DataController = __webpack_require__(71);

var _DataController2 = _interopRequireDefault(_DataController);

var _writtenNumber = __webpack_require__(149);
...
var PlayerController = function () {
	function PlayerController(data) {
	...
	(0, _createClass3.default)(PlayerController, [{
		key: '_dispatchPlayerEvent',
		value: function _dispatchPlayerEvent(event) {
			var data = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;

			var e = new CustomEvent('playerEvent', {
				detail: {
					event: event,
					data: data
				}
		...
		key: '_userEvent',
		value: function _userEvent(e) {
			var action = e.detail.action;
			var sessionNumber = e.detail.data;
		...
		value: function _initialize(data) {
			...
			this._dataSrc = new _DataController2.default(data);
		}
	}, {
		key: 'play',
		value: function play() {
			var sessionNumber = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this._dataSrc.lastSessionPosition;

			this._stopped = false;
			var session = this._dataSrc.getSession(sessionNumber);
			this._dataSrc.nowPlaying = sessionNumber;

			this._dispatchPlayerEvent('playing', sessionNumber);

			var utterances = session.numbers.map(function (num, i) {
				console.log((0, _writtenNumber2.default)(num));
				var msg = null;

				if ((session.lang === 'en-UK' || session.lang === 'en-US') && num >= 1000000) {
					// English speech synthesis doesn't support long numbers
					msg = new SpeechSynthesisUtterance((0, _writtenNumber2.default)(num));
				} else {
		...
		value: function createSession() {
			var session = this._dataSrc.createSession();
		...
		key: '_speechEngine',
		value: function _speechEngine(utterances) {
			var _marked = /*#__PURE__*/_regenerator2.default.mark(genSpeech);

			var gen = genSpeech(this, utterances); // ugly, think how to fix
			...
			function genSpeech(that, speech) {
				...
				return _regenerator2.default.wrap(function genSpeech$(_context2) {
					while (1) {
			...
			function executor(gen) {
				...
				if (!span.done) {
					span.value.then(stopIfPaused.bind(this)).then(function (result) {
						if (_this2._stopped) {
							return;
						}
						executor.call(_this2, gen);
					}, function (reject) {
						console.log('Error');
					});
				} else {
...
var _StateController = __webpack_require__(72);

var _StateController2 = _interopRequireDefault(_StateController);

var _ViewController = __webpack_require__(73);
...
var UserActionController = function () {
	function UserActionController(appContainer) {
		(0, _classCallCheck3.default)(this, UserActionController);

		this._appContainer = appContainer;
		this._display = appContainer.querySelector('.js-displayResult');
		this._state = new _StateController2.default(appContainer);
		this._view = new _ViewController2.default(appContainer);

		this._initializeListeners();
	}

	(0, _createClass3.default)(UserActionController, [{
		key: '_initializeListeners',
		value: function _initializeListeners() {
			var _this = this;

			document.addEventListener('click', function (e) {
				var btn = e.target.closest('.js-controls');

				if (btn && !btn.disabled) {

					var sessionPosition = _this._view.getPosition(btn.closest('.js-session')); // returns -1 in case of error
					var action = btn.dataset.action;

					_this._dispatchUserEvent(action, sessionPosition);
				}
			});
			...
			document.addEventListener('playerEvent', function (e) {
				switch (e.detail.event) {
					case 'playing':
						_this._state.setState('playing', e.detail.data);
						_this._view.activateSession(e.detail.data);
						break;
					case 'playingSpan':
						_this._view.activateSpan(e.detail.data);
						break;
					case 'stopped':
						_this._state.setState('stopped', e.detail.data);
						_this._view.deactivateSession(e.detail.data);
						break;
					case 'paused':
						_this._state.setState('paused', e.detail.data);
						break;
					case 'resumed':
						_this._state.setState('playing', e.detail.data);
						break;
					case 'deleteAll':
						_this._view.resetAll();
						_this._state.setState('reset');
						break;
					case 'supportError':
						_this._view._supportError();
						break;
					case 'newSession':
						var session = _this._view.createSession(e.detail.data.numbers, e.detail.data.viewData);
						var playNew = new CustomEvent('userEvent', {
							detail: {
								action: 'play',
								data: session
							}
						});
						_this._dispatchPlayEvent(playNew);
				}
			});
		}
	}, {
		key: '_dispatchUserEvent',
		value: function _dispatchUserEvent(action, data) {

			var e = new CustomEvent('userEvent', {
				detail: {
					action: action,
					data: data
				}
		...
		key: '_dispatchPlayEvent',
		value: function _dispatchPlayEvent(e) {
			...
			if (this._state.getState() === 'paused') {
				...
				// no session, main play button clicked - create session first

				var createSession = new CustomEvent('userEvent', {
					detail: {
...
exports.default = UserActionController;
...
var _FormController = __webpack_require__(74);

var _FormController2 = _interopRequireDefault(_FormController);

var _helperFunctions = __webpack_require__(41);
...
var FormValidator = function () {
	function FormValidator(container) {
		(0, _classCallCheck3.default)(this, FormValidator);

		this._formCtrl = new _FormController2.default(container);
	...
	(0, _createClass3.default)(FormValidator, [{
		key: 'getData',
		value: function getData() {
			var fields = this._formCtrl.getFieldsData();
			var data = {
				min: this._inputValues.min.value,
				max: this._inputValues.max.value,
				quantity: this._inputValues.quantity.value
			};
			return (0, _assign2.default)(data, fields);
		}
	}, {
		key: '_addListeners',
		value: function _addListeners() {
			...
			document.addEventListener('keydown', function (e) {
				if (e.target.classList.contains('js-numbers')) {
					if (e.key === '-') {
						e.preventDefault();
						_this._changeSign(e.target.name);
					} else {
						_this._previousInput = e.target.value;
					}
				}
			});
			...
			document.addEventListener('change', function (e) {
				...
				if (e.target.closest('.js-separ')) {
					...
					_this._formatInputFields();
				}
			});

			(0, _keys2.default)(this._inputs).forEach(function (input) {
				_this._inputs[input].addEventListener('blur', function (e) {
					_this._inputValidate(e);
				});
			});
			...
			document.addEventListener('playerEvent', function (e) {
				switch (e.detail.event) {
					case 'playing':
					case 'supportError':
						_this._formCtrl.blockForm();
						break;
					case 'stopped':
						_this._formCtrl.unBlockForm();
						break;
				}
			});
			...
			document.addEventListener('click', function (e) {
				if (e.target.classList.contains('js-switchButton')) {
					e.preventDefault();
					_this.switchMinMax();
					_this._formCtrl.removeSwitchError();
				}
			});

			document.addEventListener('change', function (e) {
				if (e.target.classList.contains('js-lang')) {
					var lang = _this._formCtrl.lang;

					if (lang === 'ja-JP') {
						_this._formCtrl.showSpacing();
					} else {
						_this._formCtrl.hideSpacing();
						_this._formCtrl._spacingDefault();
					}
				}
			});
		}
		...
		key: 'updateInput',
		value: function updateInput(input, value) {
			var separator = this._formCtrl.separator;
			value = (0, _helperFunctions.formatNumber)(value, separator);
		...
		key: 'restoreInput',
		value: function restoreInput(input, value) {
			this._formCtrl.setInput(input, value);
		}
	}, {
		key: '_formatInputFields',
		value: function _formatInputFields() {
			var _this2 = this;

			(0, _keys2.default)(this._inputValues).forEach(function (input) {
				_this2.updateInput(input, _this2._inputValues[input].value);
			});
		}
	}, {
		key: '_changeSign',
		value: function _changeSign(field) {
			var newValueStr = this._formCtrl.getInput(field);
			var newValueInt = null;
			var _inputValues$field = this._inputValues[field],
			    min = _inputValues$field.min,
			    max = _inputValues$field.max,
			    value = _inputValues$field.value;
		...
		key: '_inputNumber',
		value: function _inputNumber(e) {
			var field = e.target.name;
			var separator = this._formCtrl.separator;
			var _inputValues$field2 = this._inputValues[field],
			    min = _inputValues$field2.min,
			    max = _inputValues$field2.max,
			    value = _inputValues$field2.value;

			var newValueStr = this._formCtrl.getInput(field);
			var newValueInt = +newValueStr.split(separator).join('');
		...
		key: '_inputValidate',
		value: function _inputValidate(e) {
		...
		key: 'switchMinMax',
		value: function switchMinMax() {
			var temp = this._inputValues.min.value;
...
var DataController = function () {
	function DataController(form) {
		(0, _classCallCheck3.default)(this, DataController);
	...
	(0, _createClass3.default)(DataController, [{
		key: "getSession",
		value: function getSession(pos) {
		...
		value: function createSession() {
			var session = {};
			var settings = this._form.getData();
		...
		key: "_generateNumberList",
		value: function _generateNumberList(_ref) {
...
var StateController = function () {
	function StateController(container) {
		var stateString = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'reset';
	...
	(0, _createClass3.default)(StateController, [{
		key: 'getState',
		...
		key: 'setState',
		value: function setState(stateString, activeSession) {
			this._state = stateString;
		...
		value: function _playing() {
			...
			// block all buttons except for the active session, switch play button to pause button
		...
		key: '_paused',
		value: function _paused() {
			var active = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : -1;
			// switch main active session pause button to play button
			this._ctrlBtnsUpToDate();
		...
		value: function _stopped() {
			var active = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : -1;
			// switch play button to pause button; unblock all buttons
			this._ctrlBtnsUpToDate();
		...
		key: '_getCtrlBtns',
		value: function _getCtrlBtns() {
			this._ctrlBtns.play = (0, _from2.default)(this._display.querySelectorAll('[data-action="play"]'));
			this._ctrlBtns.pause = (0, _from2.default)(this._display.querySelectorAll('[data-action="pause"]'));
			this._ctrlBtns.stop = (0, _from2.default)(this._display.querySelectorAll('[data-action="stop"]'));
		}
	}, {
		key: '_ctrlBtnsUpToDate',
		value: function _ctrlBtnsUpToDate() {
		...
		key: '_ctrlBtnsBlock',
		value: function _ctrlBtnsBlock(active) {

			if (active === -1) {
				return;
			}

			for (var i = 0; i < this._ctrlBtns.play.length; i++) {
				if (i !== active) {
		...
		key: '_ctrlBtnsUnblock',
		value: function _ctrlBtnsUnblock() {
			(0, _from2.default)(this._display.querySelectorAll('button:disabled')).forEach(function (btn) {
				btn.disabled = false;
			});
		}
	}, {
		key: '_switchPauseToPlay',
		value: function _switchPauseToPlay(active) {
			if (active === -1) {
				return;
			}
		...
		key: '_switchPlayToPause',
		value: function _switchPlayToPause(active) {
			if (active === -1) {
				return;
			}
...
var _sessionTemplate = __webpack_require__(137);

var _sessionTemplate2 = _interopRequireDefault(_sessionTemplate);

var _errorTemplate = __webpack_require__(134);
...
var ViewController = function () {
	function ViewController(container) {
		...
		document.addEventListener('userEvent', function (e) {
			if (e.detail.action === 'wrap') {
				_this._wrapSession(e.detail.data);
			}
		});
	}
	...
	(0, _createClass3.default)(ViewController, [{
		key: 'getPosition',
		value: function getPosition(session) {
		...
		key: 'createSession',
		value: function createSession(numberArray) {
			var _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {
			    ...
			    japStyle = _ref.japStyle;

			var article = document.createElement('article');
			article.classList.add('js-session');

			var position = this._counter;
			this._counter++;

			var numbers = numberArray.map(function (num) {
				return (0, _helperFunctions.formatNumber)(num, separator, japStyle);
			});
			article.innerHTML = (0, _lodash2.default)(_sessionTemplate2.default)({ num: this._counter, numbers: numbers, wrapped: wrapped });
		...
		key: 'activateSession',
		value: function activateSession(num) {
			var session = this._display.children[num];

			if (session) {
				session.classList.add('js-active');

				var spans = session.querySelector('.js-numbers').children;
		...
		key: 'activateSpan',
		value: function activateSpan(num) {
			this.deactivateSpan();
		...
		value: function deactivateSession(num) {
			this.deactivateSpan();
		...
		key: '_wrapSession',
		value: function _wrapSession(num) {

			var session = this._sessions[num];
			var btns = session.querySelectorAll('[data-action="wrap"]');
			var list = session.querySelector('.js-numbers');
			...
			if (btns) {
				(0, _from2.default)(btns).forEach(function (btn) {
		...
		key: '_supportError',
		value: function _supportError() {
			this._errorDisplay.innerHTML = _errorTemplate2.default;
		}
	}]);
...
var _formTemplate = __webpack_require__(135);

var _formTemplate2 = _interopRequireDefault(_formTemplate);

var _languageSelectTemplate = __webpack_require__(136);

var _languageSelectTemplate2 = _interopRequireDefault(_languageSelectTemplate);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

// a map preserves the necessary order
var LANG = new _map2.default([['en-GB', 'English UK'], ['en-US', 'English US'], ['fr-FR', 'French'], ['de-DE', 'German'], ['it-IT', 'Italian'], ['ja-JP', 'Japanese'], ['pl-PL', 'Polish'], ['pt-BR', 'Portuguese'], ['ru-RU', 'Russian'], ['es-ES', 'Spanish']]);

var DEFAULT_SPACING = '3';

var FormController = function () {
	function FormController(container) {
		(0, _classCallCheck3.default)(this, FormController);

		this._form = this._createForm(container);
		this._inputs = new _map2.default([['min', this._form['min']], ['max', this._form['max']], ['quantity', this._form['quantity']]]);
		this._errorMessage = this._form.querySelector('.js-error');
		this._spacing = this._form.querySelector('.js-digitGroup');
	}

	(0, _createClass3.default)(FormController, [{
		key: 'getInputsData',
		value: function getInputsData() {
			...
			try {
				for (var _iterator = (0, _getIterator3.default)(this._inputs), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
		...
		value: function getInputElements() {
			...
			try {
				for (var _iterator2 = (0, _getIterator3.default)(document.querySelectorAll('.js-numbers')), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
		...
		key: 'setInput',
		value: function setInput(input, value) {
		...
		key: 'removeSwitchError',
		value: function removeSwitchError() {
		...
		key: '_spacingDefault',
		value: function _spacingDefault() {
			var _iteratorNormalCompletion3 = true;
			var _didIteratorError3 = false;
			var _iteratorError3 = undefined;

			try {
				for (var _iterator3 = (0, _getIterator3.default)(this._form['digitGroup']), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
		...
		key: '_createForm',
		value: function _createForm(container) {
			var form = document.createElement('form');
			form.name = "js-userInput";
			form.innerHTML = (0, _lodash2.default)(_formTemplate2.default)();
		...
		key: '_languageDropList',
		value: function _languageDropList() {
			var _this = this;
			...
			return new _promise2.default(function (resolve, reject) {
				...
				var supportedLang = new _set2.default();
				voices.forEach(function (voice) {
					return supportedLang.add(voice.lang);
				});
				...
				try {
					for (var _iterator4 = (0, _getIterator3.default)(LANG), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
				...
				var langSelect = (0, _lodash2.default)(_languageSelectTemplate2.default)({
					arr: arr
				});
				var dropList = _this._form.querySelector('.js-lang');
				if (dropList) {
		...
		key: 'spacing',
		get: function get() {
			var selector = this._form.querySelector('input[name="digitGroup"]:checked');
...
var _FormValidator = __webpack_require__(70);

var _FormValidator2 = _interopRequireDefault(_FormValidator);

var _PlayerController = __webpack_require__(67);

var _PlayerController2 = _interopRequireDefault(_PlayerController);

var _UserActionController = __webpack_require__(68);

var _UserActionController2 = _interopRequireDefault(_UserActionController);

var _polyfills = __webpack_require__(69);
...
var app = new _UserActionController2.default(document.getElementById('js-numTrainer'));
var validator = new _FormValidator2.default(document.querySelector('.js-userInput'));
var player = new _PlayerController2.default(validator);
...
module.exports = "<div>\n<p>Unfortunately your browser is not supported.</p>\n</p>The app should normally run in the latest versions of Chrome (full support), Firefox or Opera (with some limitations to the number of languages).</p>\n</div>";

/***/ }),
/* 135 */
/***/ (function(module, exports) {

module.exports = "<fieldset class='js-input-form'>\n\t<div class=\"form-group row\">\n\t\t\n\t\t<div class=\"col-md-6 radioGroup\">\n\t\t\t<div class=\"col-md-4\">\n\t\t\t\t<label>Digit Separator: </label>\n\t\t\t</div>\n\t\t\t<div class=\"col-md-8\">\n\t\t\t\t<label class=\"radio-inline js-separ\"><input  class=\"form-check-input\" id=\"separatorSpace\" name=\"digitSep\" value=\"&nbsp;\" type=\"radio\" checked> Space</label>\n\t\t\t\t<label class=\"radio-inline js-separ\"><input  class=\"form-check-input\" id=\"separatorComma\" name=\"digitSep\" value=\",\" type=\"radio\"> Comma</label>\n\t\t\t\t<label class=\"radio-inline js-separ\"><input  class=\"form-check-input\" id=\"separatorPoint\" name=\"digitSep\" value=\".\" type=\"radio\"> Point</label>\n\t\t\t\t<label class=\"radio-inline js-separ\"><input  class=\"form-check-input\" id=\"separatorNone\" name=\"digitSep\" value=\"\" type=\"radio\"> None</label>\n\t\t\t</div>\n\t\t</div>\n\t\t<div class=\"col-md-6 radioGroup js-digitGroup js-invisible\">\n\t\t\t<div class=\"col-md-4\">\n\t\t\t\t<label>Spacing between: </label>\n\t\t\t</div>\n\t\t\t<div class=\"col-md-8\">\n\t\t\t\t<label class=\"radio-inline js-grouping\"><input  class=\"form-check-input\" id=\"threeDigits\" name=\"digitGroup\" value=\"3\" type=\"radio\" checked> 3 Digits</label>\n\t\t\t\t<label class=\"radio-inline js-grouping\"><input  class=\"form-check-input\" id=\"fourDigits\" name=\"digitGroup\" value=\"4\" type=\"radio\"> 4 Digits</label>\n\t\t\t</div>\n\t\t</div>\n\t</div>\n\t<div class=\"form-group row\">\n\t\t<div class=\"col-md-6\">\n\t\t\t<div class=\"col-md-8\">\n\t\t\t\t<label for=\"min\">Minimum value:</label>\n\t\t\t</div>\n\t\t\t<div class=\"col-md-4\">\n\t\t\t\t<input id=\"min\" class=\"form-control js-numbers\" type=\"text\" name=\"min\" data-min=\"-100000000\" data-max=\"100000000\" value=\"10000\">\n\t\t\t</div>\n\t\t</div>\n\t\t<div class=\"col-md-6\">\n\t\t\t<div class=\"col-md-8\">\n\t\t\t\t<label for=\"max\">Maximum value:</label>\n\t\t\t</div>\n\t\t\t<div class=\"col-md-4\">\n\t\t\t\t<input id=\"max\" class=\"form-control js-numbers\" type=\"text\" name=\"max\" data-min=\"-100000000\" data-max=\"100000000\" value=\"1000000\">\n\t\t\t</div>\n\t\t</div>\n\t</div>\n\t<div class=\"form-group row\">\n\t\t<div class=\"col-md-6\">\n\t\t\t<div class=\"col-md-8\">\n\t\t\t\t<label for=\"quantity\">How many numbers do you want to be generated:</label>\n\t\t\t</div>\n\t\t\t<div class=\"col-md-4\">\n\t\t\t\t<input id=\"quantity\" class=\"form-control js-numbers\" type=\"text\" name=\"quantity\" data-min=\"1\" data-max=\"1000\" value=\"10\">\n\t\t\t</div>\n\t\t</div>\n\t\t<div class=\"col-md-6\">\n\t\t\t<div class=\"col-md-8\">\n\t\t\t\t<label for=\"rate\">Speaking rate:</label>\n\t\t\t</div>\n\t\t\t<div class=\"col-md-4\">\n\t\t\t\t<input id=\"rate\" type=\"range\" name=\"rate\" min=\"1\" max=\"2\" value=\"1\" step=\"0.01\">\n\t\t\t</div>\n\t\t</div>\n\t</div>\n\t<div class=\"form-group row\">\n\t\t<div class=\"col-md-6\">\n\t\t\t<div class=\"col-md-8\">\n\t\t\t\t<label for=\"displayCurrent\">Display numbers: </label>\n\t\t\t</div>\n\t\t\t<div class=\"col-md-4\">\n\t\t\t\t<div class=\"smallInput\"><input  class=\"form-check-input\" id=\"displayCurrent\" name=\"displayCurrent\" type=\"checkbox\" checked></div>\n\t\t\t</div>\n\t\t</div>\n\t\t<div class=\"col-md-6\">\n\t\t\t<div class=\"col-md-8\">\n\t\t\t\t<label for=\"lang\">Language:</label>\n\t\t\t</div>\n\t\t\t<div class=\"col-md-4\">\n\t\t\t\t<select class='js-lang' id=\"lang\" name=\"lang\">\n\t\t\t\t\t<option value=\"en-US\" selected>Loading</option>\n\t\t\t\t</select>\n\t\t\t</div>\n\t\t</div>\n\t</div>\n</fieldset>\n<div class=\"row\">\n\t<div class=\"col-md-12\">\n\t\t<p class=\"js-error text-center js-invisible\">Min. should be greater than max. <button class=\"btn btn-secondary js-switchButton\" data-action=\"switchVals\">Switch them?</button></p>\n\t</div>\n</div>";

/***/ }),
/* 136 */
/***/ (function(module, exports) {

module.exports = "<% arr.forEach( lang => {%>\n<option value=\"<%-lang[0]%>\"<% if (lang[0] === 'en-US'){%>selected <%}%>><%-lang[1]%></option>\n<%})%>\n\n\n<!-- <% arr.forEach( (lang, abbr) => {%>\n<option value=\"<%-lang%>\"<% if (abbr === 'en-US'){%>selected <%}%>><%-lang%></option>\n<%})%> -->";

/***/ }),
/* 137 */
/***/ (function(module, exports) {

module.exports = "<div class='clearfix'>\n\t<div class=\"heading\">\n\t<h2 class=\"js-sessionNumber\">Session <%-num%>:</h2>\n\t<div class=\"controls\">\n\t<button class=\"js-controls btn btn-secondary btn-sm\" data-action=\"play\"><i class=\"fa fa-play\" aria-hidden=\"true\"></i></button>\n\t<button class=\"js-controls btn btn-secondary btn-sm js-hidden\" data-action=\"pause\"><i class=\"fa fa-pause\" aria-hidden=\"true\"></i></button>\n\t<button class=\"js-controls btn btn-secondary btn-sm\" data-action=\"stop\"><i class=\"fa fa-stop\" aria-hidden=\"true\"></i></button>\n\t</div>\n\t<% if ( !wrapped ){%>\n\t<button class=\"js-controls btn btn-secondary btn-sm js-hidden wrapBtn\" data-action=\"wrap\"><i class=\"fa fa-angle-down\" aria-hidden=\"true\"></i></button>\n\t<button class=\"js-controls btn btn-secondary btn-sm wrapBtn\" data-action=\"wrap\"><i class=\"fa fa-angle-up\" aria-hidden=\"true\"></i></button>\n\t<%} else {%>\n\t<button class=\"js-controls btn btn-secondary btn-sm wrapBtn\" data-action=\"wrap\"><i class=\"fa fa-angle-down\" aria-hidden=\"true\"></i></button>\n\t<button class=\"js-controls btn btn-secondary btn-sm js-hidden wrapBtn\" data-action=\"wrap\"><i class=\"fa fa-angle-up\" aria-hidden=\"true\"></i></button>\n\t<%}%>\n\t</div>\n\t<p class=\"js-numbers clearfix slide\" class=\"newClass\">\n\t\t<% for (let i = 0; i < numbers.length; i++) { %>\n\t\t\t<span><%-numbers[i]%></span>\n\t\t<% } %>\n\t</p>\n</div>";
...
var i18n = {
  en: __webpack_require__(142),
  es: __webpack_require__(144),
  pt: __webpack_require__(147),
  fr: __webpack_require__(145),
  eo: __webpack_require__(143),
  it: __webpack_require__(146),
  vi: __webpack_require__(148),
  enIndian: __webpack_require__(141)
};
exports.i18n = i18n;
